...
class Regridder:
    ...
    def _regrid_dataarray(self, data):
        ...
        if self.weights is None:
            weights_da, ind_da = self.get_weights()

        def regrid_chunk(chunk):
            data_flat = chunk.flatten()

            # Determine coordinate names for 'lon' and 'lat'
            lon_name, lat_name = self.determine_coordinate_names(data)

            # Get the indices of 'lon' and 'lat' coordinates
            lon_indices = data.get_axis_num(lon_name)
            lat_indices = data.get_axis_num(lat_name)

            # Create a tuple of slice objects to index the 'lon' and 'lat' dimensions
            slices = [slice(None)] * data.ndim
            slices[lon_indices] = slice(None)
            slices[lat_indices] = slice(None)

            # Apply regridding along 'lon' and 'lat' dimensions
            regridded_chunk = data_flat[self.weights.values.flatten()]
            regridded_chunk = regridded_chunk.reshape(data.shape)

            return regridded_chunk[tuple(slices)]
    ...
    def regrid(self, data):
        ...
        if 'lon' not in data or 'lat' not in data:
            raise KeyError("Missing 'lon' or 'lat' coordinates in the input data.")
    ...
    def save_weights(self, filename):
        """
        Saves the computed weights to a NetCDF file with error handling.
    ...
    def load_weights(self, filename):
        """
        Loads the weights from a NetCDF file with error handling.
